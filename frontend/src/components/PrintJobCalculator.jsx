...
import { convertToEURSync, formatEURPrice, convertExtraCostToEUR } from '../utils/currencyConverter';
  ...
  // Calculate extras costs
    ...
    if (job.isBookletMode) {
      ...
      const coverExtrasResults = job.hasCover && selectedCoverExtras.length > 0 
        ? calculateVariantExtrasCost(job, selectedCoverExtras, coverBindingEdge, 'cover', { coverResults })
        : [];
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Main Binding Edge Selection - only for normal mode */}
            {!jobData.isBookletMode && (
                        ...
                        <div className="space-y-2">
                          {results.extrasResults.coverExtras.map((extra, index) => (
                            <div key={index} className="flex justify-between items-center p-2 border rounded bg-white">
                              ...
                              <div className="text-right">
                                <span className="font-semibold text-green-600">€{convertToEURSync(extra.totalCost, extra.originalPrice?.currency || 'EUR').toFixed(2)}</span>
                              </div>
                            </div>
                          ))}
                          ...
                          <div className="flex justify-between items-center pt-2 border-t">
                            ...
                            <span className="font-bold text-green-600">
                              €{results.extrasResults.coverExtras.reduce((sum, extra) => sum + convertToEURSync(extra.totalCost, extra.originalPrice?.currency || 'EUR'), 0).toFixed(2)}
                            </span>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="space-y-2">
                          {results.extrasResults.innerExtras.map((extra, index) => (
                            <div key={index} className="flex justify-between items-center p-2 border rounded bg-white">
                              ...
                              <div className="text-right">
                                <span className="font-semibold text-orange-600">€{convertToEURSync(extra.totalCost, extra.originalPrice?.currency || 'EUR').toFixed(2)}</span>
                              </div>
                            </div>
                          ))}
                          ...
                          <div className="space-y-2">
                            {results.extrasResults.map((extra, index) => (
                              <div key={index} className="flex justify-between items-center p-2 border rounded bg-white">
                                <div className="flex-1">
                                  ...
                                  <div className="text-xs text-gray-600 mt-1">
                                    {extra.pricingType === 'per_page' && `${extra.units} ${extra.unitType} × €${convertToEURSync(extra.pricePerUnit, extra.originalPrice?.currency || 'EUR').toFixed(2)} per page`}
                                    {extra.pricingType === 'per_booklet' && `${extra.units} ${extra.unitType} × €${convertToEURSync(extra.pricePerUnit, extra.originalPrice?.currency || 'EUR').toFixed(2)} per unit`}
                                    {extra.pricingType === 'per_length' && (
                                      `${extra.units} ${extra.unitType} × ${extra.edgeLength?.toFixed(1) || '0.0'}cm edge × €${convertToEURSync(extra.pricePerUnit, extra.originalPrice?.currency || 'EUR').toFixed(2)} per cm (${lengthBasedEdge} edge)`
                                    )}
                                  </div>
                                </div>
                                <div className="text-right">
                            ...
                            <div className="flex justify-between items-center pt-2 border-t">
                              ...
                              <span className="font-bold text-purple-600">
                                €{results.extrasResults.reduce((sum, extra) => sum + convertToEURSync(extra.totalCost, extra.originalPrice?.currency || 'EUR'), 0).toFixed(2)}
                              </span>
                            </div>
                          </div>
                        </div>
