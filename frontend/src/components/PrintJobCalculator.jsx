                          ...
                          <div>
                            ...
                            <Select value={selectedCoverExtraId} onValueChange={(value) => {
                              ...
                              <SelectContent>
                                {extras
                                  .filter(extra => {
                                    // First, check bookletApplicationScope - only show if applicable to cover
                                    if (extra.bookletApplicationScope === 'inner_only') {
                                      return false; // Don't show inner_only extras in cover section
                                    }
                                    
                                    // Then apply existing insideOutsideSame logic
                                    // If has cover, show all remaining extras including insideOutsideSame ones in cover section
                                    if (jobData.hasCover) {
                                      return true; // Show all remaining extras in cover section
                                    } else {
                        ...
                        <div className="space-y-2">
                          {results.extrasResults.coverExtras.map((extra, index) => (
                            <div key={index} className="flex justify-between items-center p-2 border rounded bg-white">
                              ...
                              <div className="text-right">
                                <span className="font-semibold text-green-600">{formatUserPrice(extra.totalCost, extra.originalPrice?.currency || 'EUR')}</span>
                              </div>
                            </div>
                          ))}
                          ...
                          <div className="flex justify-between items-center pt-2 border-t">
                            ...
                            <span className="font-bold text-green-600">
                              {formatUserPriceEUR(results.extrasResults.coverExtras.reduce((sum, extra) => sum + convertToEURSync(extra.totalCost, extra.originalPrice?.currency || 'EUR'), 0))}
                            </span>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="space-y-2">
                          {results.extrasResults.innerExtras.map((extra, index) => (
                            <div key={index} className="flex justify-between items-center p-2 border rounded bg-white">
                              ...
                              <div className="text-right">
                                <span className="font-semibold text-orange-600">{formatUserPrice(extra.totalCost, extra.originalPrice?.currency || 'EUR')}</span>
                              </div>
                            </div>
                          ))}
                          ...
                          <div className="flex justify-between items-center pt-2 border-t">
                            ...
                            <span className="font-bold text-orange-600">
                              {formatUserPriceEUR(results.extrasResults.innerExtras.reduce((sum, extra) => sum + convertToEURSync(extra.totalCost, extra.originalPrice?.currency || 'EUR'), 0))}
                            </span>
                          </div>
                        </div>
                      </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex justify-between items-center pt-2 border-t">
                              ...
                              <span className="font-bold text-purple-600">
                                {formatUserPriceEUR(results.extrasResults.reduce((sum, extra) => sum + convertToEURSync(extra.totalCost, extra.originalPrice?.currency || 'EUR'), 0))}
                              </span>
                            </div>
                          </div>
                        </div>
                      ...
                      {results.job.isBookletMode ? (
                            ...
                            return (
                              <>
                                ...
                                <div className="text-center text-sm text-gray-600 mt-3">
                                  <p>Price per booklet (EUR): {formatUserPriceEUR(eurCosts.totalEUR / results.job.quantity)}</p>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between items-center p-4 bg-blue-100 rounded-lg border-2 border-blue-300">
                            <span className="text-xl font-bold text-blue-900">Grand Total (EUR):</span>
                            <span className="text-2xl font-bold text-blue-800">
                              {formatUserPriceEUR((displayResults[0]?.totalCost || 0) + 
